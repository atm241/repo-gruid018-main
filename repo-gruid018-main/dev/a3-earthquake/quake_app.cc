...
void QuakeApp::OnGlobeBtnPressed() {
    ...
    earth_.DrawGlobeMode(globe_mode_);
}
...
void QuakeApp::DrawUsingOpenGL() {
    ...
    int mag_range = quake_db_.max_magnitude() - quake_db_.min_magnitude();

    if (globe_mode_) {
       // model_matrix = model_matrix * (Matrix4::RotationY(GfxMath::ToRadians(5)));
        ...
        for (int x = start; x < end; x++) {
            ...
            // Draw earthquake
            float quakelong = GfxMath::ToRadians(e.longitude());
            float quakelat = GfxMath::ToRadians(e.latitude());
            float quakemag = e.magnitude() - quake_db_.min_magnitude();

            float magmod = (quakemag / mag_range);
            
            

            Matrix4 quake_matrix = Matrix4::Translation((earth_.LatLongToSphere(quakelat, quakelong) - (Point3::Origin()))) * Matrix4::Scale(Vector3(0.12*magmod, 0.12*magmod, 0.12*magmod));
            quick_shapes_.DrawSphere(quake_matrix, view_matrix_, proj_matrix_, Color(magmod, 0.22, 0));
        }
    }
    else {
        int start = quake_db_.FindMostRecentQuake(Date(current_time_ - PLAYBACK_WINDOW));
        ...
        for (int x = start; x < end; x++) {
            ...
            Matrix4 quake_matrix = Matrix4::Translation((earth_.LatLongToPlane(quakelat, quakelong) - (Point3::Origin()))) * Matrix4::Scale(Vector3(0.12*magmod, 0.12*magmod, 0.12*magmod));
            quick_shapes_.DrawSphere(quake_matrix, view_matrix_, proj_matrix_, Color(magmod, 0.22, 0));
    }
