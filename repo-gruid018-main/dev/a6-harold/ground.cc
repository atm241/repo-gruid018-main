                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    for (int i = 0; i < stroke2d.size(); i++) {
        ...
        Point3 p;
        float t; 
        eyeThroughMouse.IntersectPlane(S, normal, &t, &p);
    
        flat_curve.push_back(p);
    }
    ...
    for (int i = 0; i < ground_mesh_.num_vertices(); i++) {
        ...
        float h = hfunc(normal, flat_curve, P.ClosestPointOnPlane(S, normal));
