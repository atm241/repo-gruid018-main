                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
    ...
    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
        ...
        // quick_shapes_.DrawCylinder(ctm, view_matrix, proj_matrix, Color(1.0, 0.4, 1.0));
        // quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(1,0.4,1), Point3(0,0,0), Point3(0,0,0) + skeleton_.BoneDirectionAndLength(bone_name), 0.02);
        quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.1f, 0.14f, 0.095f)), view_matrix, proj_matrix, Color(0.1, 0.8, 0.9));
    }
    if (bone_name == "lfemur" || bone_name == "rfemur") {
        // quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0.1, 0.1, 0.9), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.02);
    }
    ...
    if (bone_name == "lfoot" || bone_name == "rfoot") {
        // quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.02);
    }
    ...
    if (bone_name == "lowerback") {
        // quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0.1, 0.8, 0.9), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.1);
    }
    ...
    if (bone_name == "lowerneck" || bone_name == "upperneck") {
        // quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0.95, 0.7, 0.6), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.02);
    }
    if (bone_name == "head") {
        // quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
        //quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(1, 0.4, 1), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.02);
        ...
        //arnold head
        quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.3f, 0.12f, 0.15f)), view_matrix, proj_matrix, Color(0.95, 0.7, 0.6));
        quick_shapes_.DrawSphere(ctm * Matrix4::Translation(Vector3(0.32f, 0.0f, 0.0f)) * Matrix4::Scale(Vector3(0.03f, 0.03f, 0.03f)), view_matrix, proj_matrix, Color(0.95, 0.7, 0.6));
        quick_shapes_.DrawSphere(ctm * Matrix4::Translation(Vector3(-0.32f, 0.0f, 0.0f)) * Matrix4::Scale(Vector3(0.03f, 0.03f, 0.03f)), view_matrix, proj_matrix, Color(0.95, 0.7, 0.6));
        quick_shapes_.DrawSphere(ctm * Matrix4::Translation(Vector3(0.0f, 0.02f, 0.15f)) * Matrix4::Scale(Vector3(0.04f, 0.04f, 0.04f)), view_matrix, proj_matrix, Color(0.95, 0.7, 0.6));
        //eyes
        quick_shapes_.DrawSphere(ctm * Matrix4::Translation(Vector3(0.18f, 0.02f, 0.12f)) * Matrix4::Scale(Vector3(0.035f, 0.035f, 0.035f)), view_matrix, proj_matrix, Color(1.0, 1.0, 1.0));
        quick_shapes_.DrawSphere(ctm * Matrix4::Translation(Vector3(-0.18f, 0.02f, 0.12f)) * Matrix4::Scale(Vector3(0.035f, 0.035f, 0.035f)), view_matrix, proj_matrix, Color(1.0, 1.0, 1.0));
        //arnold hat
        quick_shapes_.DrawSphere(ctm * Matrix4::Translation(Vector3(0.0f, 0.1f, 0.0f)) * Matrix4::Scale(Vector3(0.05f, 0.07f, 0.05f)), view_matrix, proj_matrix, Color(0.1, 0.8, 0.9));
        quick_shapes_.DrawCube(ctm * Matrix4::Translation(Vector3(0.0f, 0.11f, 0.04f)) * Matrix4::Scale(Vector3(0.045f, 0.005f, 0.045f)), view_matrix, proj_matrix, Color(0.1, 0.8, 0.9));
        //arnold classic spikey hair
        quick_shapes_.DrawCone(ctm * Matrix4::Translation(Vector3(0.08f, 0.15f, 0.0f)) * Matrix4::RotationZ(GfxMath::ToRadians(-10)) * Matrix4::Scale(Vector3(0.07, 0.175, 0.04)), view_matrix, proj_matrix, Color(0.9, 0.8, 0.05));
        quick_shapes_.DrawCone(ctm * Matrix4::Translation(Vector3(0.15f, 0.13f, 0.0f)) * Matrix4::RotationZ(GfxMath::ToRadians(-15)) * Matrix4::Scale(Vector3(0.07, 0.175, 0.04)), view_matrix, proj_matrix, Color(0.9, 0.8, 0.05));
        quick_shapes_.DrawCone(ctm * Matrix4::Translation(Vector3(0.22f, 0.11f, 0.0f)) * Matrix4::RotationZ(GfxMath::ToRadians(-20)) * Matrix4::Scale(Vector3(0.07, 0.175, 0.04)), view_matrix, proj_matrix, Color(0.9, 0.8, 0.05));
    ...
    if (bone_name == "lclavicle" || bone_name == "rclavicle") {
        // quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0.1, 0.8, 0.9), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.02);
    }
    ...
    if (bone_name == "lhand" || bone_name == "rhand" || bone_name == "lthumb" || bone_name == "rthumb" || bone_name == "rfingers" || bone_name == "lfingers") {
       // quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0.95, 0.7, 0.6), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.01);
    }
